...
После создания базового функционала началось "свободное плавание" по улучшению производительности и архитектуры.

Я решил заменить bruteforce broadphase на sweep and prune, чтобы улучшить производительность и закрыть давний гештальт.
...
т.к. по ним мы очень много итерируемся во время решения контактов, и это наверняка приводит к cache miss-ам, т.к. они не размещены непрерывно в памяти. Как вариант можно попробовать кастомный аллокатор. (UPD: реализовано, unordred_map пока остается, но contact manifolds теперь располагаются в векторе)
...
3. "Волшебная" модулярная арифметика в narrow phase вроде `(pointIndex + 3 - 3 * fi) % 4`. Я такое ой как люблю, но понимаю, что буду сам это с трудом смогу дешифровывать спустя буквально пару недель.
