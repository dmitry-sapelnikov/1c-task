...
```cpp
 ...
 e1 |        | e3
    ...
    v2 ---- v3
        e2
```
Имеется в оригинальной версии у Catto, когда реализовывал OOBB vs OOBB, понял, что без него реально никуда.
...
нормальной статьи или туториала с "каноническим" решением, все интерпретировали алгоритм по-разному.
...
1. Разграничение подсистем коллизий и динамики (ну точнее не совсем полное, т.к. collision system пока знает о телах, но это несложно будет исправить используя ECS, см. ниже)
2. Максимальное использование contiguous-memory контейнеров, единственная "паршивая овца" - `unordered_map` для contact manifolds. Да, я ввел ограничение на преаллокацию тел в `World`, но, как мне кажется, это того стоило, плюс не очень сложно добавить алгоритм для ремаппинга указателей на тела, если вдруг всё же потребуется снять ограничение.
...
# Обратная связь
Вопросы, комментарии и конструктивная критика очень и очень приветствуются.
